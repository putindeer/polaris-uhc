...
@Scenario(name = "FortuneBoys", author = "putindeer", icon = Material.GOLDEN_PICKAXE,
        description = "Every tool has Fortune II.",
        incompatibleWith = {ScenarioType.FORTUNE_BABIES, ScenarioType.FORTUNE_BOYS_PLUS})
public class FortuneBoys extends BaseScenario {
    @EventHandler
    public void onClick(InventoryClickEvent event) {
        ItemStack item = event.getCurrentItem();
        if (item == null) return;
        if (!plugin.utils.isTool(item)) return;
        if (item.getEnchantmentLevel(Enchantment.FORTUNE) >= 2) return;
